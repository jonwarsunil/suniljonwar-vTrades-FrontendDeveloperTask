import React, { useState } from 'react';
import { useField } from 'formik';

const Input = ({ label, type = 'text', ...props }) => {
  const [showPassword, setShowPassword] = useState(false);
  const isPassword = type === 'password';

  const [field, meta] = useField(props);

  return (
    <div className='mb-6 w-full max-w-[385px]'>
      {label && (
        <label className='block text-[12px] leading-[20px] tracking-[-0.02em] text-white font-normal mb-2'>
          {label}
        </label>
      )}
      <div className='relative'>
        <input
          {...field}
          {...props}
          type={isPassword && showPassword ? 'text' : type}
          className={`w-full h-[50px] px-4 pr-12 rounded-[10px] bg-[#1D1E26] text-white border ${
            meta.touched && meta.error ? 'border-red-500' : 'border-[#30303D]'
          } focus:outline-none focus:ring-2 focus:ring-[#8B5CF6]`}
        />
        {isPassword && (
          <button
            type='button'
            onClick={() => setShowPassword(!showPassword)}
            className='absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-white cursor-pointer'
          >
            {showPassword ? (
              // Eye-off icon
              <svg width='18' height='18' viewBox='0 0 18 18' fill='none' xmlns='http://www.w3.org/2000/svg'>
                <g clipPath='url(#clip0_13_238)'>
                  <path
                    d='M13.455 13.455C12.1729 14.4323 10.6118 14.9737 9 15C3.75 15 0.75 9.00002 0.75 9.00002C1.68292 7.26144 2.97685 5.74247 4.545 4.54502M7.425 3.18002C7.94125 3.05918 8.4698 2.99877 9 3.00002C14.25 3.00002 17.25 9.00002 17.25 9.00002C16.7947 9.85172 16.2518 10.6536 15.63 11.3925M10.59 10.59C10.384 10.8111 10.1356 10.9884 9.85962 11.1114C9.58362 11.2343 9.28568 11.3005 8.98357 11.3058C8.68146 11.3111 8.38137 11.2555 8.10121 11.1424C7.82104 11.0292 7.56654 10.8608 7.35289 10.6471C7.13923 10.4335 6.9708 10.179 6.85763 9.89881C6.74447 9.61865 6.6889 9.31856 6.69423 9.01645C6.69956 8.71434 6.76568 8.4164 6.88866 8.1404C7.01163 7.86441 7.18894 7.616 7.41 7.41002'
                    stroke='white'
                    strokeWidth='1.3'
                    strokeLinecap='round'
                    strokeLinejoin='round'
                  />
                  <path
                    d='M0.75 0.75L17.25 17.25'
                    stroke='white'
                    strokeWidth='1.3'
                    strokeLinecap='round'
                    strokeLinejoin='round'
                  />
                </g>
                <defs>
                  <clipPath id='clip0_13_238'>
                    <rect width='18' height='18' fill='white' />
                  </clipPath>
                </defs>
              </svg>
            ) : (
              // Eye icon
              <svg
                xmlns='http://www.w3.org/2000/svg'
                className='h-5 w-5'
                fill='none'
                viewBox='0 0 24 24'
                stroke='currentColor'
                strokeWidth='2'
              >
                <path strokeLinecap='round' strokeLinejoin='round' d='M15 12a3 3 0 11-6 0 3 3 0 016 0z' />
                <path
                  strokeLinecap='round'
                  strokeLinejoin='round'
                  d='M2.458 12C3.732 7.943 7.523 5 12 5c4.477 0 8.268 2.943 9.542 7-1.274 4.057-5.065 7-9.542 7-4.477 0-8.268-2.943-9.542-7z'
                />
              </svg>
            )}
          </button>
        )}
      </div>

      {meta.touched && meta.error && <p className='text-red-500 text-sm mt-1'>{meta.error}</p>}
    </div>
  );
};

export default Input;
